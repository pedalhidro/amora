...
# ── Navegador e-ink (guidão DIY) ──────────────────────────────────────────
# GET /eink/map.png — quadro 1-bit pronto pro dispositivo blitar. Modos:
#   ?route=<slug>               visão geral da rota (enquadra sozinho)
#   ?lat=&lng=&z=               enquadramento manual
#   ?follow=<apelido|token>     centraliza na pessoa ao vivo (o GPS é o
#                               telefone dela, via Localização ao vivo)
# Combináveis (route dá o traçado grosso; follow dá o centro). Extras:
#   w=/h= (px, default 400×300) · fmt=png|raw (raw = framebuffer Waveshare)
#   base=osm|hydro|none (ruas | raster de hidrografia | só vetores) · invert=1
# Renderização em backend/eink.py; guia completo em docs/EINK_NAVIGATOR.md.
# Pillow é import lazy: sem o pacote, o resto do backend segue de pé.
@app.get("/eink/map.png")
def get_eink_map():
    try:
        import eink
    except ImportError:
        return jsonify(error="Pillow ausente (pip install pillow)"), 501
    ...
    try:
        routes = _load_routes_payload().get("routes") or []
    except Exception:  # noqa: BLE001 — mapa sem rotas ainda é um mapa
        routes = []
    ...
    with _live_positions_lock:
        _prune_live(now)
        for t, p in _live_positions.items():
            peers.append({"id": t, "name": p.get("name"), "lat": p["lat"],
                          "lng": p["lng"], "heading": p.get("heading"),
                          "trail": list(p.get("trail") or [])})
    ...
    if (lat is None or lng is None) and route_slug:
        ...
        lat, lng, zfit = eink.fit_route(r["latlngs"], min(w, eink.MAX_W),
                                        min(h, eink.MAX_H))
        if z is None:
            z = zfit
    ...
    body, mime = eink.image_to_response_bytes(img, fmt)
